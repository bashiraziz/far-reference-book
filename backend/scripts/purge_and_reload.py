"""
Script to purge Qdrant collection and reload all FAR Parts (1-53).

This script:
1. Deletes the existing Qdrant collection
2. Creates a fresh collection
3. Populates it with all FAR parts 1-53
"""
...
def iter_document_chunks(docs_dir: Path, start_part: int = 1, end_part: int = 53) -> Generator[Dict[str, Any], None, None]:
...
async def main():
    ...
    logger.info("=== PURGE AND RELOAD: FAR Vector Database ===\n")
    ...
    logger.info("Step 1: Deleting existing Qdrant collection...")
    try:
    ...
    logger.info("\nStep 2: Creating fresh Qdrant collection...")
    VectorStoreService.create_collection()
    logger.info("✓ Collection created successfully")

    # Step 3: Populate with all parts
    logger.info("\nStep 3: Populating with ALL FAR parts (1-53)...")
    chunk_iterator = iter_document_chunks(docs_dir, start_part=1, end_part=53)
    ...
    logger.info(f"\nQdrant database has been purged and reloaded with all FAR parts (1-53)!")
