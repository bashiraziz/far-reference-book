"""Upload Part 53 to Qdrant production database - FINAL PART!"""
import asyncio
import sys
...
def iter_document_chunks(part_num: int, docs_dir: Path) -> Generator[Dict[str, Any], None, None]:
    """Yield chunks for a specific part."""
    part_dir = docs_dir / f"part-{part_num}"
    if not part_dir.exists():
        logger.warning(f"Directory not found: {part_dir}")
        return

    logger.info(f"Processing part-{part_num} (FINAL PART!)...")
...
async def main():
    """Upload Part 53 to production database - complete the FAR!"""
    logger.info("=== Uploading Part 53 to Production Database ===")
    logger.info("=== COMPLETING THE ENTIRE FAR DATABASE! ===\n")
    ...
    chunk_iterator = iter_document_chunks(53, docs_dir)
    ...
    if fail_count == 0:
        ...
        logger.info("The COMPLETE Federal Acquisition Regulation (FAR)")
        logger.info("is now loaded in your production database!")
        logger.info("All 53 parts are ready for semantic search!")
        logger.info("="*60)
