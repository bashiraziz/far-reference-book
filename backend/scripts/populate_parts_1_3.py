"""
Script to populate Qdrant vector database with FAR Parts 1-3.
...
INGEST_CHUNK_SIZE = settings.chunk_size  # Now 600 chars
INGEST_CHUNK_OVERLAP = settings.chunk_overlap  # Now 150 chars
...
def iter_document_chunks(docs_dir: Path, start_part: int = 1, end_part: int = 3) -> Generator[Dict[str, Any], None, None]:
...
async def main():
    ...
    logger.info("=== FAR Vector Database Population (Parts 1-3) ===\n")
    ...
    logger.info("\nProcessing FAR documents (Parts 1-3)...")
    chunk_iterator = iter_document_chunks(docs_dir, start_part=1, end_part=3)
