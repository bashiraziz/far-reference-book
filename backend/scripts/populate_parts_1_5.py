"""
Script to populate Qdrant with FAR Parts 1-5 (test run).

This is a smaller test to verify the system works before loading all parts.
"""
...
def iter_document_chunks(docs_dir: Path, start_part: int = 1, end_part: int = 5) -> Generator[Dict[str, Any], None, None]:
...
async def upload_to_qdrant(chunks_iter, batch_size: int = 50):
    ...
    for chunk in chunks_iter:
        ...
        if len(batch) >= batch_size:
            ...
            fail_count += failed
            logger.info(f"  Batch {batch_number} complete ({success} succeeded, {failed} failed)")
            batch_number += 1
            batch = []
    ...
    if batch:
        ...
        success_count += success
        fail_count += failed
        logger.info(f"  Final batch complete ({success} succeeded, {failed} failed)")
...
async def main():
    ...
    logger.info("=== FAR Vector Database Population (Parts 1-5 TEST) ===\n")
    ...
    logger.info("Step 1: Deleting existing collection...")
    try:
    ...
    logger.info("\nStep 2: Creating fresh collection...")
    VectorStoreService.create_collection()
    logger.info("Collection created successfully")

    logger.info("\nStep 3: Populating with FAR Parts 1-5...")
    chunk_iterator = iter_document_chunks(docs_dir, start_part=1, end_part=5)
    ...
    logger.info(f"\nFAR Parts 1-5 have been loaded into Qdrant!")
