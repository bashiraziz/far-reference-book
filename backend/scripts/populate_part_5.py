"""
Script to populate Qdrant vector database with FAR Part 5 ONLY.

This script specifically targets Part 5 to get section 5.101 loaded quickly.
"""
...
INGEST_CHUNK_SIZE = settings.chunk_size  # 600 chars
INGEST_CHUNK_OVERLAP = settings.chunk_overlap  # 150 chars
...
def extract_metadata_from_path(file_path: Path) -> Dict[str, Any]:
    ...
    chapter = int(chapter_match.group(1)) if chapter_match else 5
...
def iter_document_chunks(docs_dir: Path) -> Generator[Dict[str, Any], None, None]:
    """Yield chunks for Part 5 only."""
    part_dir = docs_dir / "part-5"
    if not part_dir.exists():
        logger.error(f"Directory not found: {part_dir}")
        return

    logger.info(f"Processing part-5...")
...
async def main():
    ...
    logger.info("=== FAR Vector Database Population (Part 5 ONLY) ===\n")
    ...
    logger.info("\nProcessing FAR Part 5 (includes section 5.101)...")
    ...
    logger.info(f"\n✓ Part 5 loading complete! Section 5.101 should now be searchable.")
