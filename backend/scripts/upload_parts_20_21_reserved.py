"""Upload Parts 20-21 reserved placeholders to Qdrant production database."""
import asyncio
import sys
...
def iter_document_chunks(docs_dir: Path, start_part: int, end_part: int) -> Generator[Dict[str, Any], None, None]:
    ...
    for part_num in range(start_part, end_part + 1):
        ...
        logger.info(f"Processing part-{part_num} (reserved)...")
...
async def main():
    """Upload Parts 20-21 reserved placeholders to production database."""
    logger.info("=== Uploading Parts 20-21 Reserved Placeholders to Production Database ===\n")
    ...
    logger.info("Note: Parts 20-21 are reserved sections with placeholder content\n")

    chunk_iterator = iter_document_chunks(docs_dir, start_part=20, end_part=21)
    ...
    logger.info("Users will now see 'Reserved' messages when searching for Parts 20-21")
