"""
Script to populate Qdrant vector database with FAR Parts 30-52 (excluding Part 35).
...
# Use configured chunk size (600 chars, 150 overlap)
...
def iter_document_chunks(docs_dir: Path, start_part: int = 30, end_part: int = 52) -> Generator[Dict[str, Any], None, None]:
    """Yield chunks lazily so we never hold everything in memory."""
    for part_num in range(start_part, end_part + 1):
        # Skip Part 35 (incomplete source file)
        if part_num == 35:
            logger.info(f"Skipping part-{part_num} (no section files available)")
            continue
...
async def main():
    ...
    logger.info("=== FAR Vector Database Population (Parts 30-52, excluding Part 35) ===\n")
    ...
    logger.info(f"\nChunk size: {INGEST_CHUNK_SIZE} chars")
    logger.info(f"Chunk overlap: {INGEST_CHUNK_OVERLAP} chars\n")

    logger.info("Processing FAR documents (Parts 30-34, 36-52)...")
    chunk_iterator = iter_document_chunks(docs_dir, start_part=30, end_part=52)
